...
change: `feature branch -> implement -> verify -> applicable redaction -> commit
...
mandatory for every published change — Docs, Code, Tests, Config, Refactor,
Bugfix, Feature, Dependency, and CI/CD — and none may direct-push around the
gate.
...
- Commit and push from a non-default branch, then create or update one PR for that coherent change. Direct default-branch delivery is not an allowed workflow path. When the branch carries Plan metadata, validate it; when it does not, publish normally — a persisted Plan is a planning decision, not a publication precondition.
...
- `feature_branch_required`: local changes are on the default branch; create a
  feature branch before committing or pushing.
...
After a successful branch push, check for an existing PR before creating
...
- Require one PR per coherent published change, including fixes, documentation, configuration, tests, dependencies, and CI/CD changes; keep related tests and documentation in that PR.
