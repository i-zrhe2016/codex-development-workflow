...
- For every feature, fix, documentation, configuration, dependency, test, or CI/CD change, create or resume a feature branch different from the repository's default branch before editing. Commit the completed unit, push that branch, and then create or update one PR for the unit. Direct default-branch delivery is not an allowed workflow path.
...
After a successful feature-branch push, check for an existing PR before creating
